//! Faces to matte: the face rows arriving with each frame are rasterized into
//! a grayscale mask - 255 inside each box, 0 everywhere else. `grow` pads
...
//!
//! The rows are `ages`'s rows, the six box fields with the estimated age
//! beside them, and the age is not read here: which faces to cover is settled
//! before the rows arrive, by whatever the query filtered them on. This is the
//! detector's own `boxes_mask` carried over for that one reason - the rows it
//! declares are the six-field record, and a seven-field row is not that.
//!
...
/// One box to rasterize, as `ages` reports it. Extra keys are ignored: a row
/// carrying a class, a confidence and an age beside the four coordinates is
/// still a box.
#[derive(Deserialize)]
struct Rect {
    x: f64,
...
fn parse_params(params: &str) -> Result<Params, String> {
    ...
    let parsed: Params = if trimmed.is_empty() {
        ...
        serde_json::from_str(trimmed)
            .map_err(|e| format!("faces_mask cannot read its params: {e}"))?
    };
    for (name, value) in [("grow", parsed.grow), ("feather", parsed.feather)] {
        if !value.is_finite() || !(0.0..=4096.0).contains(&value) {
            return Err(format!(
                "faces_mask needs {name} between 0 and 4096, got {value}"
            ));
        }
    }
    Ok(parsed)
...
impl Guest for FacesMask {
    fn describe() -> WindowMeta {
        WindowMeta {
            meta: Meta {
                name: "faces_mask".to_string(),
    ...
    fn init(format: Format, _stream_info: StreamInfo, params: String) -> Result<(), String> {
        let Format::Video(video) = format else {
            return Err("faces_mask reads frames, and this stream is audio".to_string());
        };
        let pix_fmt = match video.pix_fmt.as_str() {
            "yuv420p" => PixFmt::Yuv420p,
            "rgba" => PixFmt::Rgba,
            other => return Err(format!("faces_mask does not accept pixel format {other}")),
        };
...
mod tests {
    ...
    fn a_box_paints_hard_edges_when_nothing_feathers() {
        let map = rasterize(
            &rows(&[r#"{"class":"face","conf":0.9,"x":10,"y":10,"w":20,"h":10,"age":8.4}"#]),
            W,
            H,
            params(0.0, 0.0),
        );
