-- Mosaic the children and leave everyone else alone: every face the detector
-- finds is aged, and the ones estimated under `max_age` are mosaicked. `max_age`
...
-- line go with it.
-- variables: source (input media path), max_age (faces estimated younger than this are mosaicked; required, 16 is what this recipe was built for), conf (confidence threshold, defaults to 0.25), grow (pixels added around each box, defaults to 8), feather (how far the edge softens in pixels, defaults to 4), size (mosaic block size, defaults to 16), hold (frames a face the detector lost is carried for, defaults to 5), every (frames between one face's scores, defaults to 1), track (video track index, defaults to the first), dest (output path)
-- example: ffrwd compile -f packages/ffrwd/faceage/recipes/mosaic-children.sql -v source=class.mp4 -v max_age=16 -v dest=mosaic.mp4
COPY (
